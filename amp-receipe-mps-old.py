...
def end_timer_and_print(local_msg):
    ...
    print("Current mem used by tensors = {} bytes".format(torch.mps.current_allocated_memory()))
...
with profile(activities=[
        ...
        ProfilerActivity.CUDA  # NOTE: Despite saying CUDA, this is used for GPU activities incl. Metal on M1/M2
    ],
    with_stack=True,
    record_shapes=True,
    profile_memory=True
...
for epoch in range(0): # 0 epochs, this section is for illustration only
    for input, target in zip(data, targets):
        ...
        # fxl: dtype only chooses between float16 or bf16
...
scaler = torch.amp.GradScaler("cuda")   # fxl, new api
